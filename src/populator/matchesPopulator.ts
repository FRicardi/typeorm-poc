...
export default async function populateMatches() {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    for (var j = 0; j < matches[i].rounds.length; j++) {
      ...
      let existingRound = await roundRepository.findOneBy({
        worldCup: worldCup,
        name: round.name,
      });

      if (!existingRound) {
        existingRound = await roundRepository.save({
          worldCup: worldCup,
          name: round.name,
        });
      }
      ...
      for (var k = 0; k < round.matches.length; k++) {
        ...
        if (!existingMatch) {
          console.log(
            `Inserting ${match.team1.code} x ${match.team2.code} on ${worldCup.name}`
          );
          const country1 = await countryRepository.findOneBy({
            acronym: match.team1.code,
          });
          const country2 = await countryRepository.findOneBy({
            acronym: match.team2.code,
          });
          let stadium = null;

          if (!!match.stadium) {
            stadium = await stadiumRepository.findOneBy({
              name: match.stadium.name,
            });
          } else {
            console.log(
              `stadium for ${match.team1.code} x ${match.team2.code} on ${worldCup.name} not found`
            );
          }
          ...
          console.log(
            `Match ${match.team1.code} x ${match.team2.code} on ${worldCup.name} already exists`
          );
        }
      }
    }
